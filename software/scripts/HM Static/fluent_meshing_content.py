...
### 外空气域流体边界层
#### 获取目标实体所有边的id
all_edge_ids = get_entity_ids('edge', 'ambient_air')
#### 读取现有Name selection
selected_ids = []
for ns in Model.NamedSelections.Children:
...
## 外环境域网格
### 网格划分方法
body_id = get_entity_ids('body', 'ambient_air')
...
ns = DataModel.GetObjectsByName(ns_lists[2])[0]
inflation.BoundaryLocation = ns
...
unneed_id = get_entity_ids('body','firebrick')[0]
